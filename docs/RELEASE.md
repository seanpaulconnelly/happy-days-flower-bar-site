...
| 7   | DNSimple records + verification TXT (§2.2, §2.3)             | 🧑 Sean         | **done** — ALIAS apex, CNAME `www`, `_github-pages-challenge` TXT all verified at public resolvers; domain verified on the GitHub account |
| 8   | GitHub custom domain + Enforce HTTPS (§2.4)                  | 🧑 Sean         | **done** — custom domain saved; DNS check successful; certificate covering apex + www active 2026-08-29 ~20:10 UTC; **Enforce HTTPS: re-tick once the checkbox re-enables** (it resets while a cert is provisioning) |
| 9   | `happydaysflowerfarm.com` URL forwarding (§2.5)              | 🧑 Sean         | pending (B4)                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 10  | Post-cutover verification (§2.6)                             | agent           | **done** 2026-08-29 20:13 UTC — apex 200 with valid cert, `https://www` → 301 apex, `index,follow`, canonical, JSON-LD, robots/sitemap/llms, base-aware 404, github.io → apex 301, axe 0, no overflow; pending: `http://` → `https://` redirect after Enforce HTTPS is re-ticked |
