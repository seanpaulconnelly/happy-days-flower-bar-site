...
| Q4  | 2026-08-29 | ui-designer → frontend-engineer | **Bucket clipPath (signature element).** Inline the `<svg width="0" height="0">` `<clipPath id="bucket">` from `docs/design-spec.md` §10 once in `App.tsx`; apply `clip-path: url(#bucket)` only to the three Why Happy Days images. Mark the Custom band and the Inquiry section with `data-surface="brand"` so the focus ring switches to warm white (`theme.css` base layer).                                                                                          | part done (Phase 5a) — `<clipPath id="bucket">` inlined once in `App.tsx`; `data-surface="brand"` on the `#inquire` section. The Why trio (5c) and the custom band (5c) apply it |
